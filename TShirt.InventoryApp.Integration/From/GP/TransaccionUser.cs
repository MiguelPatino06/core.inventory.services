...
{
	class TransaccionUser : Transaccion, ITransaccion
	{
		private string mItmbs;

		public TransaccionUser(string strcon, string nin, string nout, string hin, string hout, string itbms)
			: base("TransaccionUser", 4, strcon, nin, nout, hin, hout,itbms)
		{
			mItmbs = itbms;
		}
		...
		private DataTable getSalespersons()
		{
			...
			command.CommandType = CommandType.Text;
			command.CommandText = "SELECT * FROM GR_Employee";
