...
{
    class TransactionMakeReturn : Transaccion, ITransaccion
    {
        private string file;
        ...
        public TransactionMakeReturn(string strcon, string nin, string nout, string hin, string hout,string fil,string srv, string datab,string itbms)
            : base("TransactionMakeReturn", 10, strcon, nin, nout, hin, hout,itbms)
        { file = fil;
        ...
        private void GenerateDocument()
        {
            ...
            string eConnectConnectionString = string.Format("Integrated Security=SSPI;Persist Security Info=False;Initial Catalog={1};Data Source={0};", this.server, this.db);
                ...
                string DOCID = GPHelper.returnDOCID(this.StringConnection).Trim();
            ...
            if (data.Tables["HEADER"].Rows.Count > 0 && data.Tables["LINES"].Rows.Count > 0)
            {
                ...
                NodeElement.InnerText = "4"; // RETURN
                ...
                foreach(DataRow item in data.Tables["LINES"].Rows)
                {
                    ...
                    NodeElement = eConnectXMLDocument.CreateElement("QTYRTRND");
                    NodeElement.InnerText = item["QUANTITY"].ToString();
                    taSopLineNode.AppendChild(NodeElement);
                        ...
                        switch (n.Name.Trim().ToUpper())
                        {
                            case "DOCAMNT":
                                if (n.InnerText == "")
                                    ...
                                    lnerow["CHEKBKID"] = Decimal.Parse(n.InnerText.Replace(".", ".").Replace(",", "."));
                                break;
                            case "PYMTTYPE":
