...
{
    class TransactionAddUser : Transaccion, ITransaccion
    {
        private string file;
        ...
        public TransactionAddUser(string strcon, string nin, string nout, string hin, string hout, string fil, string srv, string datab, string itbms)
            : base("TransactionAddUser", 11, strcon, nin, nout, hin, hout, itbms)
        {
            file = fil;
        ...
        {
            // GenerateDocument();
            Models.User item = getFromFile(file);
            if(item!=null)
                if(InsertUser(item))
                {
                    try {
        ...
        private Models.User getFromFile(string filename)
        {
            return Models.User.LoadFrom(filename);
        }


        private bool UserExist(Models.User sp)
        {
            bool res = false;
        ...
        private bool InsertUser(Models.User sp)
        {
            bool res = false;
            ...
            if (UserExist(sp))
            {
                SqlCommand deluser = new SqlCommand("DELETE FROM [User] WHERE [Username]=@user", new SqlConnection(this.StringConnection));
                deluser.Parameters.AddWithValue("@user", sp.Code);
                deluser.CommandType = CommandType.Text;

                try
                {
                    if(deluser.Connection.State!= ConnectionState.Open)
                    {
            ...
            command = new SqlCommand("[dbo].[spCreateUser]", new SqlConnection(this.StringConnection));
            command.CommandType = CommandType.StoredProcedure;
            command.Parameters.AddWithValue("@iFirstName", sp.Firts_Name);
            command.Parameters.AddWithValue("@iSecondName", "");
            command.Parameters.AddWithValue("@iLastName", sp.Last_Name);
            command.Parameters.AddWithValue("@iUserName", sp.Code);
            command.Parameters.AddWithValue("@iPassword", sp.Password);           
            command.Parameters.AddWithValue("@iNamePrefix", "");
            command.Parameters.AddWithValue("@iNameSufix", sp.Name_of_Receipt);
            command.Parameters.AddWithValue("@iNeedChangePswd", 0);
            command.Parameters.AddWithValue("@iExpireDate", DateTime.UtcNow.AddYears(1));
        ...
        private DataSet parseXML(string filename)
        {
            ...
            data.Tables["CUSTOMER"].Columns.Add("User", typeof(string));
