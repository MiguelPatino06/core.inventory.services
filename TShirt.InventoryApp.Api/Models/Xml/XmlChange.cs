...
namespace TShirt.InventoryApp.Api.Models.Xml
{
    ...
    {
        private readonly IProductChange _changeRepository = new ProductChangeRepository();
        public bool Create(string code)
        {
            XmlCountPlan xmlCountPlan = new XmlCountPlan();
            bool success = false;
            ...
            try
            {
                ...
                string IVDOCNBR = string.Format("CMB{0}",
                    common.getNextNumber("selSI_NextNumber", "selSI_IV40100", "selSI_IV_TYPE_ID", "CMB"));

               // OrderReqExtend master 
                  List<OrderReqDetailProduct> detail = _changeRepository.GetOrderRedDetailProducts2(code);
                //List<OrderReqDetailExtend> detail = master.Detail;
                        ...
                        new XElement("eConnect",
                            new XElement("IVInventoryTransactionType",
                                ...
                                new XElement("taIVTransactionHeaderInsert",
                                    new XElement("BACHNUMB", "CMB" + DateTime.Now.ToString("ddMMHHmmss")),
                ...
                foreach(OrderReqDetailProduct row in detail)
                {
                    int qty;
                    int adjType;
                    bool negative = true;
                    lote = "Pedido " + row.OrderReqCode; // "CONT" + DateTime.Now.ToString("dd/MM/yy");

                    #region GuardaValorPositivo Producto agregado

                    XElement lot0 = new XElement("taIVTransactionLotInsert",
                        new XElement("IVDOCNBR", IVDOCNBR),
                        new XElement("IVDOCTYP", 1),
                        new XElement("ITEMNMBR", row.ProductCodeChanged),
                        new XElement("LOTNUMBR", lote),
                        new XElement("SERLTQTY", row.Quantity.ToString()),
                        new XElement("ADJTYPE", 0),
                        new XElement("LOCNCODE", row.Warehouse.Trim()),
                        new XElement("DATERECD", dateRec),
                        new XElement("AUTOCREATELOT", 1));
                    ...
                    XElement line0 = new XElement("taIVTransactionLineInsert",
                        new XElement("IVDOCNBR", IVDOCNBR),
                        new XElement("IVDOCTYP", 1),
                        new XElement("ITEMNMBR", row.ProductCodeChanged),
                        new XElement("TRXQTY", row.Quantity.ToString()),
                    ...
                    //CODIGO PARA PONER EN NEGATIVO EL PRODUCTO

                    //XElement lot1 = new XElement("taIVTransactionLotInsert",
                    //    new XElement("IVDOCNBR", IVDOCNBR),
                    //    new XElement("IVDOCTYP", 1),
                    ...
                    //XElement line1 = new XElement("taIVTransactionLineInsert",
                    //    new XElement("IVDOCNBR", IVDOCNBR),
                    //    new XElement("IVDOCTYP", 1),
                    //    new XElement("ITEMNMBR", row.ProductCode),
                    //    new XElement("TRXQTY", "-" + row.Quantity.ToString()),
                    ...
                    //queryGroup = GetLotIV00300(row.ProductCode.Trim(), "", qty);
                    ...
                    if (negative == false)
                    {
                        ...
                        XElement line = new XElement("taIVTransactionLineInsert",
                            ...
                            new XElement("TRXQTY",
                                (negative)
                                    ? (qty * -1).ToString().Replace(".00000", "")
                                    : qty.ToString().Replace(".00000", "")),
                            new XElement("TRXLOCTN", row.Warehouse.Trim()));
                ...
                doc.Save(string.Format("{0}CMB{1}.xml", Settings.Default.XmlFolderPath, DateTime.Now.ToString("yyyyMMddTHHmmss")));
